...
This manual contains detailed documentation for the XLKit library. For a quick overview and installation, see the [main README](../README.md).
...
- [Examples](#examples)
  - [Example 1: Minimal workbook and save](#example-1-minimal-workbook-and-save)
  - [Example 2: CSV round-trip](#example-2-csv-round-trip)
  - [Example 3: Image embed and save](#example-3-image-embed-and-save)
  - [Example 4: Multi-sheet with formulas](#example-4-multi-sheet-with-formulas)
...
- [API Reference](#api-reference)
  ...
  - [Sheet API](#sheet-api)
  - [Cell & coordinate types](#cell--coordinate-types)
  - [Enums](#enums)
  - [CoreUtils](#coreutils)
  - [SecurityManager API](#securitymanager-api)
  - [ImageUtils & ImageSizingUtils](#imageutils--imagesizingutils)
  - [CSVUtils](#csvutils)
  - [XLKitError](#xlkiterror)
...
XLKit provides comprehensive error handling with specific error types. All cases are documented in the [API Reference](#xlkiterror).
...
do {
    ...
    print("Invalid coordinate: \(coord)")
} catch XLKitError.invalidRange(let range) {
    print("Invalid range: \(range)")
} catch XLKitError.fileWriteError(let message) {
    print("File write error: \(message)")
} catch XLKitError.zipCreationError(let message) {
    print("ZIP creation error: \(message)")
} catch XLKitError.xmlGenerationError(let message) {
    print("XML error: \(message)")
} catch XLKitError.securityError(let message) {
    print("Security error: \(message)")
} catch XLKitError.rateLimitExceeded(let message) {
    print("Rate limit exceeded: \(message)")
} catch XLKitError.fileSizeLimitExceeded(let message) {
    print("File too large: \(message)")
} catch XLKitError.suspiciousFileDetected(let message) {
    print("Suspicious file: \(message)")
} catch {
...
Or use Xcode's built-in formatter. All code must be formatted and pass CI before merging. See `.cursorrules` for more details.

---

## API Reference

This section lists the full public API of XLKit. All types and members are available when you `import XLKit`.
